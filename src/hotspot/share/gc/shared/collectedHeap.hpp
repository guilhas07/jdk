...
class CollectedHeap : public CHeapObj<mtGC> {
  ...
  friend class IsSTWGCActiveMark; // Block structured external access to
                                  // _is_stw_gc_active
...
private:
  ...
  // Then, set it to FillerObject after the FillerObject_klass loading is
  // complete.
...
#define ENUM_TO_STRING_CASE(value)                                             \
  ...
  static char const *toString(Name color) {
    switch (color) {
      ENUM_TO_STRING_CASE(None)
      ENUM_TO_STRING_CASE(Serial)
      ENUM_TO_STRING_CASE(Parallel)
      ENUM_TO_STRING_CASE(G1)
      ENUM_TO_STRING_CASE(Epsilon)
      ENUM_TO_STRING_CASE(Z)
      ENUM_TO_STRING_CASE(Shenandoah)
    default:
      return "UNKNOWN";
    }
  }
...
protected:
  ...
  template <typename T> static T *named_heap(Name kind) {
...
public:
  ...
  static inline size_t stack_chunk_max_size() { return _stack_chunk_max_size; }

  static inline Klass *filler_object_klass() { return _filler_object_klass; }
  ...
  // Historic gc information
  size_t free_at_last_gc() const {
    return _capacity_at_last_gc - _used_at_last_gc;
  }
  ...
  DEBUG_ONLY(bool is_in_or_null(const void *p)
                 const { return p == nullptr || is_in(p); })

  void set_gc_cause(GCCause::Cause v);
  GCCause::Cause gc_cause() { return _gc_cause; }

  oop obj_allocate(Klass *klass, size_t size, TRAPS);
  virtual oop array_allocate(Klass *klass, size_t size, int length,
                             bool do_zero, TRAPS);
  ...
  virtual MetaWord *
  satisfy_failed_metadata_allocation(ClassLoaderData *loader_data, size_t size,
                                     Metaspace::MetadataType mdtype);

  // Return true, if accesses to the object would require barriers.
  // This is used by continuations to copy chunks of a thread stack into
  // StackChunk object or out of a StackChunk object back into the thread stack.
  // These chunks may contain references to objects. It is crucial that the GC
  // does not attempt to traverse the object while we modify it, because its
  // structure (oopmap) is changed when stack chunks are stored into it.
  // StackChunk objects may be reused, the GC must not assume that a StackChunk
  // object is always a freshly allocated object.
  ...
  virtual ParallelObjectIteratorImpl *
  parallel_object_iterator(uint thread_num) {
    return nullptr;
  }
...
public:
  ...
  // GCs are free to represent the bit representation for null differently in
  // memory, which is typically not observable when using the Access API.
  // However, if for some reason a context doesn't allow using the Access API,
  // then this function explicitly checks if the given memory location contains
  // a null value.
...
public:
  ...
  ~GCCauseSetter() { _heap->set_gc_cause(_previous_cause); }
};
