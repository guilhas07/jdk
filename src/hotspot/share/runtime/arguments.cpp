...
bool PathString::set_value(const char *value, AllocFailType alloc_failmode) {
  char *new_value =
      AllocateHeap(strlen(value) + 1, mtArguments, alloc_failmode);
...
ModulePatchPath::ModulePatchPath(const char *module_name, const char *path) {
  assert(module_name != nullptr && path != nullptr,
         "Invalid module name or path value");
...
static bool match_option(const JavaVMOption *option, const char **names,
                         const char **tail, bool tail_allowed) {
...
static bool match_jfr_option(const JavaVMOption **option) {
  ...
  } else if (match_option(*option, "-XX:FlightRecorderOptions",
                          (const char **)&tail)) {
    _has_jfr_option = true;
...
bool Arguments::has_jfr_option() { return _has_jfr_option; }
#endif
...
static bool matches_property_suffix(const char *option, const char *property,
                                    size_t len) {
...
bool Arguments::is_internal_module_property(const char *property) {
  if (strncmp(property, MODULE_PROPERTY_PREFIX, MODULE_PROPERTY_PREFIX_LEN) ==
      0) {
    ...
    if (matches_property_suffix(property_suffix, ADDEXPORTS, ADDEXPORTS_LEN) ||
        ...
        matches_property_suffix(property_suffix, UPGRADE_PATH,
                                UPGRADE_PATH_LEN) ||
        matches_property_suffix(property_suffix, ENABLE_NATIVE_ACCESS,
                                ENABLE_NATIVE_ACCESS_LEN)) {
      return true;
    }
  }
  return false;
...
void Arguments::init_system_properties() {
  ...
  // Initialize the vm.info now, but it will need updating after argument
  // parsing.
  _vm_info =
  ...
  _sun_boot_library_path =
      new SystemProperty("sun.boot.library.path", nullptr, true);
  ...
  _jdk_boot_class_path_append =
      new SystemProperty("jdk.boot.class.path.append", nullptr, false, true);
 ...
 *      globals.hpp, globals_<cpu>.hpp, globals_<os>.hpp,
 * <compiler>_globals.hpp, or <gc>_globals.hpp. -XX arguments are parsed in
 * parse_argument(). -XX argument bounds checking is done in
 * check_vm_args_consistency().
 *
 * Over time -XX arguments may change. There are mechanisms to handle common
 * cases:
 *
 *      ALIASED: An option that is simply another name for another option. This
 * is often part of the process of deprecating a flag, but not all aliases need
 *               to be deprecated.
 *
 *               Create an alias for an option by adding the old and new option
 * names to the "aliased_jvm_flags" table. Delete the old variable from
 * globals.hpp (etc).
 *
 *   DEPRECATED: An option that is supported, but a warning is printed to let
 * the user know that support may be removed in the future. Both regular and
 * aliased options may be deprecated.
 *
 *               Add a deprecation warning for an option (or alias) by adding an
 * entry in the "special_jvm_flags" table and setting the "deprecated_in" field.
 *               Often an option "deprecated" in one major release will
 *               be made "obsolete" in the next. In this case the entry should
 * also have its "obsolete_in" field set.
 *
 *     OBSOLETE: An option that has been removed (and deleted from globals.hpp),
 * but is still accepted on the command line. A warning is printed to let the
 * user know that option might not be accepted in the future.
 *
 *               Add an obsolete warning for an option by adding an entry in the
 * "special_jvm_flags" table and setting the "obsolete_in" field.
 *
 *      EXPIRED: A deprecated or obsolete option that has an "accept_until"
 * version less than or equal to the current JDK version. The system will flatly
 * refuse to admit the existence of the flag. This allows a flag to die
 * automatically over JDK releases.
 *
 *               Note that manual cleanup of expired options should be done at
 * major JDK version upgrades:
 *                  - Newly expired options should be removed from the
 * special_jvm_flags and aliased_jvm_flags tables.
 *                  - Newly obsolete or expired deprecated options should have
 * their global variable definitions removed (from globals.hpp, etc) and related
 ...
 * the 3-step model adding major release numbers to the deprecate, obsolete and
 * expire columns.
 *
 * To remove internal options (e.g. diagnostic, experimental, develop options),
 * use a 2-step model adding major release numbers to the obsolete and expire
 * columns.
 *
 ...
 *
 * There are times when it is appropriate to leave a future release number as
 * undefined.
 *
...
// The special_jvm_flags table declares options that are being deprecated and/or
// obsoleted. The "deprecated_in" or "obsolete_in" fields may be set to
// "undefined", but not both. When the JDK version reaches 'deprecated_in'
// limit, the JVM will process this flag on the command-line as usual, but will
// issue a warning. When the JDK version reaches 'obsolete_in' limit, the JVM
// will continue accepting this flag on the command-line, while issuing a
// warning and ignoring the flag value. Once the JDK version reaches
// 'expired_in' limit, the JVM will flatly refuse to admit the existence of the
// flag.
//
// MANUAL CLEANUP ON JDK VERSION UPDATES:
// This table ensures that the handling of options will update automatically
// when the JDK version is incremented, but the source code needs to be cleanup
// up manually:
// - As "deprecated" options age into "obsolete" or "expired" options, the
// associated "globals"
//   variable should be removed, as well as users of the variable.
// - As "deprecated" options age into "obsolete" options, move the entry into
// the
...
static SpecialFlag const special_jvm_flags[] = {
    ...
    {"AllowRedefinitionToAddDeleteMethods", JDK_Version::jdk(13),
     JDK_Version::undefined(), JDK_Version::undefined()},
    {"FlightRecorder", JDK_Version::jdk(13), JDK_Version::undefined(),
     JDK_Version::undefined()},
    {"ZGenerational", JDK_Version::jdk(23), JDK_Version::undefined(),
     JDK_Version::undefined()},
    {"DumpSharedSpaces", JDK_Version::jdk(18), JDK_Version::jdk(19),
     JDK_Version::undefined()},
    {"DynamicDumpSharedSpaces", JDK_Version::jdk(18), JDK_Version::jdk(19),
     JDK_Version::undefined()},
    {"RequireSharedSpaces", JDK_Version::jdk(18), JDK_Version::jdk(19),
     JDK_Version::undefined()},
    {"UseSharedSpaces", JDK_Version::jdk(18), JDK_Version::jdk(19),
     JDK_Version::undefined()},
    {"RegisterFinalizersAtInit", JDK_Version::jdk(22), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"DontYieldALot", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"OldSize", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"PreserveAllAnnotations", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"UseNotificationThread", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"UseEmptySlotsInSupers", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
#if defined(X86)
    {"UseRTMLocking", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"UseRTMDeopt", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
    {"RTMRetryCount", JDK_Version::jdk(23), JDK_Version::jdk(24),
     JDK_Version::jdk(25)},
#endif // X86
    // --- Deprecated alias flags (see also aliased_jvm_flags) - sorted by
    // obsolete_in then expired_in:
    {"CreateMinidumpOnCrash", JDK_Version::jdk(9), JDK_Version::undefined(),
     JDK_Version::undefined()},

    // -------------- Obsolete Flags - sorted by expired_in --------------

    {"MetaspaceReclaimPolicy", JDK_Version::undefined(), JDK_Version::jdk(21),
     JDK_Version::undefined()},

    {"G1ConcRefinementGreenZone", JDK_Version::undefined(),
     JDK_Version::jdk(20), JDK_Version::jdk(24)},
    {"G1ConcRefinementYellowZone", JDK_Version::undefined(),
     JDK_Version::jdk(20), JDK_Version::jdk(24)},
    {"G1ConcRefinementRedZone", JDK_Version::undefined(), JDK_Version::jdk(20),
     JDK_Version::jdk(24)},
    {"G1ConcRefinementThresholdStep", JDK_Version::undefined(),
     JDK_Version::jdk(20), JDK_Version::jdk(24)},
    {"G1UseAdaptiveConcRefinement", JDK_Version::undefined(),
     JDK_Version::jdk(20), JDK_Version::jdk(24)},
    {"G1ConcRefinementServiceIntervalMillis", JDK_Version::undefined(),
     JDK_Version::jdk(20), JDK_Version::jdk(24)},

    {"G1ConcRSLogCacheSize", JDK_Version::undefined(), JDK_Version::jdk(21),
     JDK_Version::jdk(24)},
    {"G1ConcRSHotCardLimit", JDK_Version::undefined(), JDK_Version::jdk(21),
     JDK_Version::jdk(24)},
    {"RefDiscoveryPolicy", JDK_Version::undefined(), JDK_Version::jdk(21),
     JDK_Version::jdk(24)},

    {"AdaptiveSizePolicyCollectionCostMargin", JDK_Version::undefined(),
     JDK_Version::jdk(23), JDK_Version::jdk(24)},
    {"MaxGCMinorPauseMillis", JDK_Version::jdk(8), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"MaxRAMFraction", JDK_Version::jdk(10), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"MinRAMFraction", JDK_Version::jdk(10), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"InitialRAMFraction", JDK_Version::jdk(10), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"DefaultMaxRAMFraction", JDK_Version::jdk(8), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"TLABStats", JDK_Version::jdk(12), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"GCLockerEdenExpansionPercent", JDK_Version::undefined(),
     JDK_Version::jdk(23), JDK_Version::jdk(24)},
    {"NUMAPageScanRate", JDK_Version::undefined(), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"ProcessDistributionStride", JDK_Version::undefined(),
     JDK_Version::jdk(23), JDK_Version::jdk(24)},

    {"ParallelOldDeadWoodLimiterMean", JDK_Version::undefined(),
     JDK_Version::jdk(23), JDK_Version::jdk(24)},
    {"ParallelOldDeadWoodLimiterStdDev", JDK_Version::undefined(),
     JDK_Version::jdk(23), JDK_Version::jdk(24)},
    {"UseNeon", JDK_Version::undefined(), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
    {"ScavengeBeforeFullGC", JDK_Version::undefined(), JDK_Version::jdk(23),
     JDK_Version::jdk(24)},
#ifdef ASSERT
    {"DummyObsoleteTestFlag", JDK_Version::undefined(), JDK_Version::jdk(18),
     JDK_Version::undefined()},
#endif

#ifdef TEST_VERIFY_SPECIAL_JVM_FLAGS
    // These entries will generate build errors.  Their purpose is to test the
    // macros.
    {"dep > obs", JDK_Version::jdk(9), JDK_Version::jdk(8),
     JDK_Version::undefined()},
    {"dep > exp ", JDK_Version::jdk(9), JDK_Version::undefined(),
     JDK_Version::jdk(8)},
    {"obs > exp ", JDK_Version::undefined(), JDK_Version::jdk(9),
     JDK_Version::jdk(8)},
    {"obs > exp", JDK_Version::jdk(8), JDK_Version::undefined(),
     JDK_Version::jdk(10)},
...
static AliasedFlag const aliased_jvm_flags[] = {
    {"CreateMinidumpOnCrash", "CreateCoredumpOnCrash"}, {nullptr, nullptr}};
    ...
    if (!flag.obsolete_in.is_undefined()) {
      if (!version_less_than(JDK_Version::current(), flag.obsolete_in)) {
        *version = flag.obsolete_in;
        // This flag may have been marked for obsoletion in this version, but we
        // may not have actually removed it yet. Rather than ignoring it as soon
        // as we reach this version we allow some time for the removal to
        // happen. So if the flag still actually exists we process it as normal,
        // but issue an adjusted warning.
...
static bool lookup_special_flag(const char *flag_name, size_t skip_index) {
  for (size_t i = 0; special_jvm_flags[i].name != nullptr; i++) {
    if ((i != skip_index) &&
        (strcmp(special_jvm_flags[i].name, flag_name) == 0)) {
      return true;
    }
  }
  return false;
...
// is out of date, with respect to the current version, then ideally a warning
// is issued but verification does not fail. This allows the VM to operate when
// the version is first updated, without needing to update all the impacted
// flags at the same time. In practice we can't issue the warning immediately
// when the version is updated as it occurs for every test and some tests are
// not prepared to handle unexpected output - see 8196739. Instead we only check
// if the table is up-to-date if the check_globals flag is true, and in addition
// allow a grace period and only check for stale flags when we hit build 25
// (which is far enough into the 6 month release cycle that all flag updates
// should have been processed, whilst still leaving time to make the change
// before RDP2). We use a gtest to call this, passing true, so that we can
// detect stale flags before the end of the release cycle.
...
bool Arguments::verify_special_jvm_flags(bool check_globals) {
  ...
  for (size_t i = 0; special_jvm_flags[i].name != nullptr; i++) {
    ...
    if (flag.deprecated_in.is_undefined() && flag.obsolete_in.is_undefined()) {
      warning("Special flag entry \"%s\" must declare version deprecated "
              "and/or obsoleted in.",
              flag.name);
      success = false;
    }
    ...
    if (!flag.deprecated_in.is_undefined()) {
      if (!version_less_than(flag.deprecated_in, flag.obsolete_in)) {
        warning(
            "Special flag entry \"%s\" must be deprecated before obsoleted.",
            flag.name);
        success = false;
      }

      if (!version_less_than(flag.deprecated_in, flag.expired_in)) {
        warning("Special flag entry \"%s\" must be deprecated before expired.",
                flag.name);
        success = false;
      }
    }
    ...
    if (!flag.obsolete_in.is_undefined()) {
      if (!version_less_than(flag.obsolete_in, flag.expired_in)) {
        warning("Special flag entry \"%s\" must be obsoleted before expired.",
                flag.name);
        success = false;
      }

      // if flag has become obsolete it should not have a "globals" flag defined
      // anymore.
      if (check_globals &&
          VM_Version::vm_build_number() >= SPECIAL_FLAG_VALIDATION_BUILD &&
          !version_less_than(JDK_Version::current(), flag.obsolete_in)) {
        if (JVMFlag::find_declared_flag(flag.name) != nullptr) {
          warning("Global variable for obsolete special flag entry \"%s\" "
                  "should be removed",
                  flag.name);
          success = false;
        }
      ...
      warning("Special flag entry \"%s\" must be explicitly obsoleted before "
              "expired.",
              flag.name);
      success = false;
    }
    ...
    if (!flag.expired_in.is_undefined()) {
      // if flag has become expired it should not have a "globals" flag defined
      // anymore.
      if (check_globals &&
          VM_Version::vm_build_number() >= SPECIAL_FLAG_VALIDATION_BUILD &&
          !version_less_than(JDK_Version::current(), flag.expired_in)) {
        if (JVMFlag::find_declared_flag(flag.name) != nullptr) {
          warning(
              "Global variable for expired flag entry \"%s\" should be removed",
              flag.name);
          success = false;
        }
      }
...
static bool set_fp_numeric_flag(JVMFlag *flag, const char *value,
...
static bool set_numeric_flag(JVMFlag *flag, const char *value,
                             JVMFlagOrigin origin) {
...
static bool set_string_flag(JVMFlag *flag, const char *value,
                            JVMFlagOrigin origin) {
  if (value[0] == '\0') {
    value = nullptr;
  }
  if (JVMFlagAccess::set_ccstr(flag, &value, origin) != JVMFlag::SUCCESS)
    return false;
...
static bool append_to_string_flag(JVMFlag *flag, const char *new_value,
                                  JVMFlagOrigin origin) {
  const char *old_value = "";
  if (JVMFlagAccess::get_ccstr(flag, &old_value) != JVMFlag::SUCCESS)
    return false;
...
const char *Arguments::handle_aliases_and_deprecation(const char *arg) {
  ...
  case 1: {
    ...
    if (real_name != arg) {
      warning("Option %s was deprecated in version %s and will likely be "
              "removed in a future release. Use option %s instead.",
...
const char *Arguments::build_resource_string(char **args, int count) {
  ...
  for (int i = 0; i < count; i++) {
    length +=
        strlen(args[i]) + 1; // add 1 for a space or null terminating character
  }
  char *s = NEW_RESOURCE_ARRAY(char, length);
  char *dst = s;
  for (int j = 0; j < count; j++) {
    size_t offset =
        strlen(args[j]) + 1; // add 1 for a space or null terminating character
    jio_snprintf(dst, length, "%s ",
                 args[j]); // jio_snprintf will replace the last space character
                           // with null character
    dst += offset;
    length -= offset;
  }
...
void Arguments::print_on(outputStream *st) {
  ...
  st->print_cr("java_command: %s",
               java_command() ? java_command() : "<unknown>");
  if (_java_class_path != nullptr) {
    char *path = _java_class_path->value();
    size_t len = strlen(path);
    st->print("java_class_path (initial): ");
    // Avoid using st->print_cr() because path length maybe longer than
    // O_BUFLEN.
    if (len == 0) {
...
bool Arguments::process_argument(const char *arg, jboolean ignore_unrecognized,
                                 JVMFlagOrigin origin) {
  JDK_Version since = JDK_Version();
  ...
  if (arg_len <= BUFLEN) {
    // Construct a string which consists only of the argument name without '+',
    // '-', or '='.
    ...
    if (is_obsolete_flag(stripped_argname, &since)) {
      char version[256];
      since.to_string(version, sizeof(version));
      warning("Ignoring option %s; support was removed in %s", stripped_argname,
              version);
      return true;
    }
  }
  ...
  const JVMFlag *found_flag =
      JVMFlag::find_declared_flag((const char *)argname, arg_len);
  if (found_flag != nullptr) {
    char locked_message_buf[BUFLEN];
    JVMFlag::MsgType msg_type =
    ...
    jio_fprintf(defaultStream::error_stream(), "Unrecognized VM option '%s'\n",
                argname);
    JVMFlag *fuzzy_matched =
        JVMFlag::fuzzy_match((const char *)argname, arg_len, true);
...
bool Arguments::process_settings_file(const char *file_name, bool should_exist,
...
bool Arguments::add_property(const char *prop, PropertyWriteable writeable,
  ...
  if (strcmp(key, "java.compiler") == 0) {
    // we no longer support java.compiler system property, log a warning and let
    // it get passed to Java, like any other system property
    if (strlen(value) == 0 || strcasecmp(value, "NONE") == 0) {
      // for applications using NONE or empty value, log a more informative
      // message
      warning("The java.compiler system property is obsolete and no longer "
              "supported, use -Xint");
    } else {
      warning("The java.compiler system property is obsolete and no longer "
              "supported.");
    ...
    if (strcmp(key, "sun.java.command") == 0) {
      ...
      // its value without going through the property list or making a Java
      // call.
    ...
    PropertyList_unique_add(&_system_properties, key, value, AddProperty,
                            writeable, internal);
  }
...
void Arguments::set_mode_flags(Mode mode) {
  ...
  PropertyList_unique_add(&_system_properties, "java.vm.info",
                          VM_Version::vm_info_string(), AddProperty,
                          UnwriteableProperty, ExternalProperty);
...
static void set_object_alignment() {
  // Object alignment.
  assert(is_power_of_2(ObjectAlignmentInBytes),
         "ObjectAlignmentInBytes must be power of 2");
...
size_t Arguments::max_heap_for_compressed_oops() {
  // Avoid sign flip.
  assert(OopEncodingHeapMax > (uint64_t)os::vm_page_size(),
         "Unusual page size");
  // We need to fit both the null page and the heap into the memory budget,
  // while keeping alignment constraints of the heap. To guarantee the latter,
  // as the null page is located before the heap, we pad the null page to the
  // conservative maximum alignment that the GC may ever impose upon the heap.
...
void Arguments::set_conservative_max_heap_alignment() {
  ...
  _conservative_max_heap_alignment =
      MAX4(heap_alignment, os::vm_allocation_granularity(), os::max_page_size(),
...
size_t Arguments::limit_heap_by_allocatable_memory(size_t limit) {
  ...
  if (os::has_allocatable_memory_limit(&max_allocatable)) {
    ...
    size_t heap_virtual_to_physical_ratio =
        (AggressiveHeap
             ? 1
             : GCConfig::arguments()->heap_virtual_to_physical_ratio());
...
void Arguments::set_heap_size() {
  ...
  bool override_coop_limit =
      (!FLAG_IS_DEFAULT(MaxRAMPercentage) ||
       ...
       !FLAG_IS_DEFAULT(InitialRAMPercentage) || !FLAG_IS_DEFAULT(MaxRAM));
  ...
  if (FLAG_IS_DEFAULT(MaxHeapSize)) {
    julong reasonable_max =
        (julong)(((double)phys_mem * MaxRAMPercentage) / 100);
    const julong reasonable_min =
        (julong)(((double)phys_mem * MinRAMPercentage) / 100);
    ...
    if (UseCompressedOops || UseCompressedClassPointers) {
      ...
      if (!FLAG_IS_DEFAULT(HeapBaseMinAddress)) {
        if (HeapBaseMinAddress < DefaultHeapBaseMinAddress) {
          ...
          log_debug(gc, heap,
                    coops)("HeapBaseMinAddress must be at least " SIZE_FORMAT
                           " (" SIZE_FORMAT
                           "G) which is greater than value given " SIZE_FORMAT,
                           DefaultHeapBaseMinAddress,
                           DefaultHeapBaseMinAddress / G, HeapBaseMinAddress);
    ...
    if (UseCompressedOops) {
      ...
      if (reasonable_max > max_coop_heap) {
        if (FLAG_IS_ERGO(UseCompressedOops) && override_coop_limit) {
          log_info(cds)("UseCompressedOops and UseCompressedClassPointers have "
                        "been disabled due to"
                        " max heap " SIZE_FORMAT
                        " > compressed oop heap " SIZE_FORMAT ". "
    ...
    log_trace(gc, heap)("  Maximum heap size " SIZE_FORMAT,
                        (size_t)reasonable_max);
    FLAG_SET_ERGO(MaxHeapSize, (size_t)reasonable_max);
  }

  // If the minimum or initial heap_size have not been set or requested to be
  // set ergonomically, set them accordingly.
  if (InitialHeapSize == 0 || MinHeapSize == 0) {
    ...
    if (InitialHeapSize == 0) {
      julong reasonable_initial =
          (julong)(((double)phys_mem * InitialRAMPercentage) / 100);
      reasonable_initial = limit_heap_by_allocatable_memory(reasonable_initial);

      reasonable_initial =
          MAX3(reasonable_initial, reasonable_minimum, (julong)MinHeapSize);
    ...
    if (MinHeapSize == 0) {
      FLAG_SET_ERGO(MinHeapSize,
                    MIN2((size_t)reasonable_minimum, InitialHeapSize));
...
jint Arguments::set_aggressive_heap_flags() {
  ...
  if (FLAG_IS_DEFAULT(NewSize)) {
    ...
    if (FLAG_SET_CMDLINE(NewSize, ((julong)MaxHeapSize / (julong)8) *
                                      (julong)3) != JVMFlag::SUCCESS) {
      return JNI_EINVAL;
    }
    ...
    !defined(AIX) // UseLargePages is not yet supported on BSD and AIX.
  FLAG_SET_DEFAULT(UseLargePages, true);
#endif

  // Increase some data structure sizes for efficiency
  if (FLAG_SET_CMDLINE(BaseFootPrintEstimate, MaxHeapSize) !=
      JVMFlag::SUCCESS) {
    return JNI_EINVAL;
  }
  ...
  if (FLAG_SET_CMDLINE(YoungPLABSize, 256 * K) !=
      JVMFlag::SUCCESS) { // Note: this is in words
    return JNI_EINVAL;
  }
  ...
  if (FLAG_SET_CMDLINE(OldPLABSize, 8 * K) !=
      JVMFlag::SUCCESS) { // Note: this is in words
    return JNI_EINVAL;
  }
...
jint Arguments::set_aggressive_opts_flags() {
  ...
  if (AggressiveUnboxing) {
    if (FLAG_IS_DEFAULT(EliminateAutoBox)) {
      ...
      // warning("AggressiveUnboxing is disabled because EliminateAutoBox is
      // disabled");
    ...
    if (FLAG_IS_DEFAULT(DoEscapeAnalysis)) {
      ...
      // warning("AggressiveUnboxing is disabled because DoEscapeAnalysis is
      // disabled");
      AggressiveUnboxing = false;
    }
  }
  if (!FLAG_IS_DEFAULT(AutoBoxCacheMax)) {
    ...
    char buffer[1024];
    jio_snprintf(buffer, 1024,
                 "java.lang.Integer.IntegerCache.high=" INTX_FORMAT,
                 AutoBoxCacheMax);
...
void Arguments::process_java_launcher_argument(const char *launcher,
                                               void *extra_info) {
...
bool Arguments::check_vm_args_consistency() {
  ...
  if (status && EnableJVMCI) {
    PropertyList_unique_add(&_system_properties, "jdk.internal.vm.ci.enabled",
                            "true", AddProperty, UnwriteableProperty,
                            InternalProperty);
    if (ClassLoader::is_module_observable("jdk.internal.vm.ci")) {
      if (!create_numbered_module_property(
              "jdk.module.addmods", "jdk.internal.vm.ci", addmods_count++)) {
        return false;
      }
    }
  }
  ...
  if (status && (FlightRecorderOptions || StartFlightRecording)) {
    if (!create_numbered_module_property("jdk.module.addmods", "jdk.jfr",
                                         addmods_count++)) {
      return false;
    }
  }
  ...
  if (StackReservedPages != 0) {
    ...
    !defined(ARM) && !defined(PPC64) && !defined(S390)
  ...
  if (LockingMode == LM_MONITOR) {
    jio_fprintf(defaultStream::error_stream(),
                "LockingMode == 0 (LM_MONITOR) is not fully implemented on "
                "this architecture\n");
    return false;
  }
#endif
  ...
  if (LockingMode == LM_MONITOR && UseRTMForStackLocks) {
    jio_fprintf(defaultStream::error_stream(),
                "LockingMode == 0 (LM_MONITOR) and -XX:+UseRTMForStackLocks "
...
bool Arguments::is_bad_option(const JavaVMOption *option, jboolean ignore,
  ...
  jio_fprintf(defaultStream::error_stream(), "Unrecognized %s%soption: %s\n",
...
bool Arguments::parse_uint(const char *value, uint *uint_arg, uint min_size) {
  uint n;
...
bool Arguments::create_module_property(const char *prop_name,
  ...
  // These are not strictly writeable properties as they cannot be set via
  // -Dprop=val. But that is enforced by checking is_internal_module_property().
  // We need the property to be writeable so that multiple occurrences of the
  // associated flag just causes the existing property value to be replaced
  // ("last option wins"). Otherwise we would need to keep track of the flags
  // and only convert to a property after we have finished flag processing.
...
bool Arguments::create_numbered_module_property(const char *prop_base_name,
                                                const char *prop_value,
                                                unsigned int count) {
  assert(is_internal_module_property(prop_base_name),
  ...
  // Make sure count is < props_count_limit. Otherwise, memory allocation will
  // be too small.
  if (count < props_count_limit) {
    size_t prop_len = strlen(prop_base_name) + strlen(prop_value) + max_digits +
                      extra_symbols_count;
    char *property = AllocateHeap(prop_len, mtArguments);
    int ret = jio_snprintf(property, prop_len, "%s.%d=%s", prop_base_name,
                           count, prop_value);
    if (ret < 0 || ret >= (int)prop_len) {
      FreeHeap(property);
      jio_fprintf(defaultStream::error_stream(),
                  "Failed to create property %s.%d=%s\n", prop_base_name, count,
                  prop_value);
      return false;
    }
  ...
  jio_fprintf(defaultStream::error_stream(),
              "Property count limit exceeded: %s, limit=%d\n", prop_base_name,
              props_count_limit);
  return false;
}
...
jint Arguments::parse_vm_init_args(const JavaVMInitArgs *vm_options_args,
  ...
  // variable (if present).
  result = parse_each_vm_init_arg(java_tool_options_args, &patch_mod_javabase,
                                  JVMFlagOrigin::ENVIRON_VAR);
  if (result != JNI_OK) {
    return result;
  }

  // Parse args structure generated from the command line flags.
  result = parse_each_vm_init_arg(cmd_line_args, &patch_mod_javabase,
                                  JVMFlagOrigin::COMMAND_LINE);
  if (result != JNI_OK) {
    return result;
  }
  ...
  result = parse_each_vm_init_arg(java_options_args, &patch_mod_javabase,
                                  JVMFlagOrigin::ENVIRON_VAR);
  if (result != JNI_OK) {
    return result;
  }
...
int Arguments::process_patch_mod_option(const char *patch_mod_tail,
  ...
  if (module_equal == nullptr) {
    jio_fprintf(defaultStream::output_stream(),
                "Missing '=' in --patch-module specification\n");
    return JNI_ERR;
  } else {
    // Pick out the module name
    size_t module_len = module_equal - patch_mod_tail;
    char *module_name =
        NEW_C_HEAP_ARRAY_RETURN_NULL(char, module_len + 1, mtArguments);
    if (module_name != nullptr) {
      ...
      *(module_name + module_len) = '\0';
      ...
      if (!create_numbered_module_property("jdk.module.patch", patch_mod_tail,
                                           patch_mod_count++)) {
        return JNI_ENOMEM;
      }
    } else {
...
jint Arguments::parse_xss(const JavaVMOption *option, const char *tail,
                          intx *out_ThreadStackSize) {
  ...
  const JVMTypedFlagLimit<intx> *limit =
      JVMFlagLimit::get_range_at(FLAG_MEMBER_ENUM(ThreadStackSize))
          ->cast<intx>();
  ...
  if (errcode != arg_in_range) {
    bool silent =
        (option == nullptr); // Allow testing to silence error messages
    if (!silent) {
      jio_fprintf(defaultStream::error_stream(),
  ...
  assert(size <= size_aligned, "Overflow: " JULONG_FORMAT " " JULONG_FORMAT,
         size, size_aligned);
  ...
  // Check that code expanding ThreadStackSize to a page aligned number of bytes
  // won't overflow.
  const julong max_expanded = align_up(size_in_K * K, os::vm_page_size());
  assert(max_expanded < max_uintx && max_expanded >= size_in_K,
         "Expansion overflowed: " JULONG_FORMAT " " JULONG_FORMAT, max_expanded,
         size_in_K);
...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs *args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    if (match_option(option, "-verbose", &tail)) {
      if (!strcmp(tail, ":class") || !strcmp(tail, "")) {
        LogConfiguration::configure_stdout(LogLevel::Info, true,
                                           LOG_TAGS(class, load));
        LogConfiguration::configure_stdout(LogLevel::Info, true,
                                           LOG_TAGS(class, unload));
      } else if (!strcmp(tail, ":module")) {
        LogConfiguration::configure_stdout(LogLevel::Info, true,
                                           LOG_TAGS(module, load));
        LogConfiguration::configure_stdout(LogLevel::Info, true,
                                           LOG_TAGS(module, unload));
      } else if (!strcmp(tail, ":gc")) {
        if (_legacyGCLogging.lastFlag == 0) {
          _legacyGCLogging.lastFlag = 1;
        }
      } else if (!strcmp(tail, ":jni")) {
        LogConfiguration::configure_stdout(LogLevel::Debug, true,
                                           LOG_TAGS(jni, resolve));
      }
      ...
      if (tail != nullptr) {
        ...
        if (pos != nullptr) {
          size_t len2 =
              strlen(pos + 1) +
              1; // options start after ':'.  Final zero must be copied.
          options = (char *)memcpy(NEW_C_HEAP_ARRAY(char, len2, mtArguments),
                                   pos + 1, len2);
        }
#if !INCLUDE_JVMTI
      ...
      if (!create_numbered_module_property("jdk.module.addreads", tail,
                                           addreads_count++)) {
        return JNI_ENOMEM;
      }
    } else if (match_option(option, "--add-exports=", &tail)) {
      if (!create_numbered_module_property("jdk.module.addexports", tail,
                                           addexports_count++)) {
        return JNI_ENOMEM;
      }
    } else if (match_option(option, "--add-opens=", &tail)) {
      if (!create_numbered_module_property("jdk.module.addopens", tail,
                                           addopens_count++)) {
        return JNI_ENOMEM;
      }
      ...
      if (!create_numbered_module_property("jdk.module.enable.native.access",
                                           tail,
                                           enable_native_access_count++)) {
        return JNI_ENOMEM;
      }
    } else if (match_option(option, "--limit-modules=", &tail)) {
      if (!create_module_property("jdk.module.limitmods", tail,
                                  InternalProperty)) {
        return JNI_ENOMEM;
      }
      ...
      if (!create_module_property("jdk.module.upgrade.path", tail,
                                  ExternalProperty)) {
        return JNI_ENOMEM;
      }
    ...
    } else if (match_option(option,
                            "--sun-misc-unsafe-memory-access=", &tail)) {
      if (strcmp(tail, "allow") == 0 || strcmp(tail, "warn") == 0 ||
          strcmp(tail, "debug") == 0 || strcmp(tail, "deny") == 0) {
        PropertyList_unique_add(
            &_system_properties, "sun.misc.unsafe.memory.access", tail,
            AddProperty, WriteableProperty, InternalProperty);
      } else {
        jio_fprintf(defaultStream::error_stream(),
                    "Value specified to --sun-misc-unsafe-memory-access not "
                    "recognized: '%s'\n",
                    tail);
        return JNI_ERR;
      }
      ...
      // -agentlib and -agentpath
               ...
               (is_absolute_path =
                    match_option(option, "-agentpath:", &tail))) {
      if (tail != nullptr) {
        const char *pos = strchr(tail, '=');
        char *name;
        if (pos == nullptr) {
      ...
      if (FLAG_SET_CMDLINE(MaxNewSize, (size_t)long_initial_young_size) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      if (FLAG_SET_CMDLINE(NewSize, (size_t)long_initial_young_size) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      // -Xms
      ...
      if (FLAG_SET_CMDLINE(MaxHeapSize, (size_t)long_max_heap_size) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      // Xmaxf
      ...
      ArgsRange errcode =
          parse_memory_size(tail, &long_ReservedCodeCacheSize, 1);
      if (errcode != arg_in_range) {
        jio_fprintf(defaultStream::error_stream(),
      ...
      if (FLAG_SET_CMDLINE(ReservedCodeCacheSize,
                           (uintx)long_ReservedCodeCacheSize) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      // -green
      ...
      if (match_option(option, "-Djava.endorsed.dirs=", &value) &&
        ...
        jio_fprintf(defaultStream::output_stream(),
                    "-Djava.endorsed.dirs=%s is not supported. Endorsed "
                    "standards and standalone APIs\n"
                    "in modular form will be supported via the concept of "
                    "upgradeable modules.\n",
                    value);
        return JNI_EINVAL;
      }
      if (match_option(option, "-Djava.ext.dirs=", &value) && *value != '\0' &&
          strcmp(value, "\"\"") != 0) {
      ...
      // Check for module related properties.  They must be set using the
      // modules options. For example: use "--add-modules=java.sql", not
      ...
      if (strcmp(tail, ":all") == 0 || strcmp(tail, "") == 0) {
        if (FLAG_SET_CMDLINE(BytecodeVerificationLocal, true) !=
            JVMFlag::SUCCESS) {
          return JNI_EINVAL;
        }
        if (FLAG_SET_CMDLINE(BytecodeVerificationRemote, true) !=
            JVMFlag::SUCCESS) {
          return JNI_EINVAL;
        }
      } else if (strcmp(tail, ":remote") == 0) {
        if (FLAG_SET_CMDLINE(BytecodeVerificationLocal, false) !=
            JVMFlag::SUCCESS) {
          return JNI_EINVAL;
        }
        if (FLAG_SET_CMDLINE(BytecodeVerificationRemote, true) !=
        ...
        if (FLAG_SET_CMDLINE(BytecodeVerificationRemote, false) !=
            JVMFlag::SUCCESS) {
          return JNI_EINVAL;
        }
        warning("Options -Xverify:none and -noverify were deprecated in JDK 13 "
      ...
      warning("Option -Xdebug was deprecated in JDK 22 and will likely be "
              "removed in a future release.");
    } else if (match_option(option, "-Xloggc:", &tail)) {
      // Deprecated flag to redirect GC output to a file. -Xloggc:<filename>
      log_warning(gc)("-Xloggc is deprecated. Will use -Xlog:gc:%s instead.",
                      tail);
      ...
      } else if (is_bad_option(option, args->ignoreUnrecognized, "check")) {
        return JNI_EINVAL;
      }
      ...
      // Need to keep consistency of MaxTenuringThreshold and
      // AlwaysTenure/NeverTenure; and the last option wins.
      ...
      if (FLAG_SET_CMDLINE(MaxTenuringThreshold, markWord::max_age + 1) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      ...
      if (FLAG_SET_CMDLINE(MaxTenuringThreshold, max_tenuring_thresh) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      ...
      if (FLAG_SET_CMDLINE(DisplayVMOutputToStdout, false) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      ...
      if (FLAG_SET_CMDLINE(DisplayVMOutputToStderr, false) !=
          JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
      ...
      if (strcmp(tail, "enabled") == 0) {
        ...
        jio_fprintf(defaultStream::error_stream(),
                    "Invalid finalization value '%s', must be 'disabled' or "
                    "'enabled'.\n",
                    tail);
        return JNI_EINVAL;
      }
      ...
      jio_fprintf(defaultStream::error_stream(),
                  "DTraceMonitorProbes flag is not applicable for this "
                  "configuration\n");
      return JNI_EINVAL;
      ...
      if (EnableJVMCIProduct) {
        jio_fprintf(defaultStream::error_stream(),
                    "-XX:-EnableJVMCIProduct or -XX:-UseGraalJIT cannot come "
                    "after -XX:+EnableJVMCIProduct or -XX:+UseGraalJIT\n");
        return JNI_EINVAL;
      }
    } else if (match_option(option, "-XX:+EnableJVMCIProduct") ||
               match_option(option, "-XX:+UseGraalJIT")) {
      ...
      // Just continue, since "-XX:+EnableJVMCIProduct" or "-XX:+UseGraalJIT"
      ...
      else if (!process_argument(use_graal_jit ? "UseGraalJIT"
                                               : "EnableJVMCIProduct",
...
void Arguments::add_patch_mod_prefix(const char *module_name, const char *path,
                                     bool *patch_mod_javabase) {
  // For java.base check for duplicate --patch-module options being specified on
  // the command line. This check is only required for java.base, all other
  // duplicate module specifications will be checked during module system
  // initialization.  The module system initialization will throw an
  // ExceptionInInitializerError if this situation occurs.
  if (strcmp(module_name, JAVA_BASE_NAME) == 0) {
    if (*patch_mod_javabase) {
      vm_exit_during_initialization("Cannot specify " JAVA_BASE_NAME
  ...
  if (_patch_mod_prefix == nullptr) {
    _patch_mod_prefix =
        new (mtArguments) GrowableArray<ModulePatchPath *>(10, mtArguments);
  }

  _patch_mod_prefix->push(new ModulePatchPath(module_name, path));
}

// Remove all empty paths from the app classpath (if IgnoreEmptyClassPaths is
// enabled)
//
// This is necessary because some apps like to specify classpath like -cp
// foo.jar:${XYZ}:bar.jar in their start-up scripts. If XYZ is empty, the
// classpath will look like "-cp foo.jar::bar.jar". Java treats such empty paths
// as if the user specified "-cp foo.jar:.:bar.jar". I.e., an empty path is
// treated as the current directory.
//
// This causes problems with CDS, which requires that all directories specified
// in the classpath must be empty. In most cases, applications do NOT want to
// load classes from the current directory anyway. Adding
// -XX:+IgnoreEmptyClassPaths will make these applications' start-up scripts
// compatible with CDS.
void Arguments::fix_appclasspath() {
  if (IgnoreEmptyClassPaths) {
    ...
    for (char *tail = copy + strlen(copy) - 1;
         tail >= copy && *tail == separator; tail--) {
      *tail = '\0';
    }
...
jint Arguments::finalize_vm_init_args(bool patch_mod_javabase) {
  ...
  jio_snprintf(path, JVM_MAXPATHLEN, "%s%slib%sendorsed",
               Arguments::get_java_home(), fileSep, fileSep);

  DIR *dir = os::opendir(path);
  if (dir != nullptr) {
    jio_fprintf(defaultStream::output_stream(),
                "<JAVA_HOME>/lib/endorsed is not supported. Endorsed standards "
                "and standalone APIs\n"
                "in modular form will be supported via the concept of "
                "upgradeable modules.\n");
    os::closedir(dir);
  ...
  jio_snprintf(path, JVM_MAXPATHLEN, "%s%slib%sext", Arguments::get_java_home(),
               fileSep, fileSep);
  dir = os::opendir(path);
  if (dir != nullptr) {
    jio_fprintf(
  ...
  // CompileThresholdScaling == 0.0 is same as -Xint: Disable compilation
  // (enable interpreter-only mode), but like -Xint, leave compilation
  // thresholds unaffected. With tiered compilation disabled, setting
  // CompileThreshold to 0 disables compilation as well.
  if ((CompileThresholdScaling == 0.0) ||
      (!TieredCompilation && CompileThreshold == 0)) {
    set_mode_flags(_int);
  }
  ...
  // eventually fix up InitialTenuringThreshold if only MaxTenuringThreshold is
  // set
  ...
  if (!CDSConfig::check_vm_args_consistency(patch_mod_javabase,
                                            mode_flag_cmd_line)) {
    return JNI_ERR;
  }
...
public:
  ...
  jint insert(const JavaVMInitArgs *args, const JavaVMInitArgs *args_to_insert,
...
jint Arguments::parse_options_environment_variable(const char *name,
  ...
  jio_fprintf(defaultStream::error_stream(), "Picked up %s: %s\n", name,
              buffer);
...
jint Arguments::parse_vm_options_file(const char *file_name,
                                      ScopedVMInitArgs *vm_args) {
...
jint Arguments::parse_options_buffer(const char *name, char *buffer,
    ...
    while (rd < buffer_end && !isspace(*rd)) {
      if (*rd == '\'' || *rd == '"') { // handle a quoted string
        ...
        if (rd < buffer_end) {
          ...
          jio_fprintf(defaultStream::error_stream(), "Unmatched quote in %s\n",
                      name);
          return JNI_ERR;
        }
      } else {
...
static bool use_vm_log() {
  if (LogCompilation || !FLAG_IS_DEFAULT(LogFile) || PrintCompilation ||
      PrintInlining || PrintDependencies || PrintNativeNMethods ||
      PrintDebugInfo || PrintRelocations || PrintNMethods ||
      PrintExceptionHandlers || PrintAssembly || TraceDeoptimization ||
...
jint Arguments::match_special_option_and_act(const JavaVMInitArgs *args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    if (match_option(option, "-XX:VMOptionsFile=", &tail)) {
      ...
      args_out->set_vm_options_file_arg(
          vm_options_file_args.vm_options_file_arg());
      if (args_out->is_set()) {
    ...
    if (match_option(option, "-XX:+PrintGCs")) {

      char const **gcs = GCConfig::get_supported_gcs();
      tty->print("Supported GCs are:\n");
      for (; *gcs != nullptr; gcs++)
        tty->print("%s\n", *gcs);
      /*JVMFlag::printFlags(tty, false);*/
      vm_exit(0);
...
bool Arguments::handle_deprecated_print_gc_flags() {
  ...
  if (_legacyGCLogging.lastFlag == 2) {
    ...
    return LogConfiguration::parse_log_arguments(_legacyGCLogging.file, gc_conf,
                                                 nullptr, nullptr, &errstream);
  } else if (PrintGC || PrintGCDetails || (_legacyGCLogging.lastFlag == 1)) {
    LogConfiguration::configure_stdout(LogLevel::Info, !PrintGCDetails,
                                       LOG_TAGS(gc));
  }
  return true;
}
...
jint Arguments::parse(const JavaVMInitArgs *initial_cmd_args) {
  assert(verify_special_jvm_flags(false),
         "deprecated and obsolete flag table inconsistent");
  JVMFlag::check_all_flag_declarations();

  // If flag "-XX:Flags=flags-file" is used it will be the first option to be
  // processed.
  ...
  jint code = parse_java_tool_options_environment_variable(
  ...
  // Parse the options in the /java.base/jdk/internal/vm/options resource, if
  // present
  char *vmoptions = ClassLoader::lookup_vm_options();
  if (vmoptions != nullptr) {
    code = parse_options_buffer("vm options resource", vmoptions,
                                strlen(vmoptions), &initial_vm_options_args);
  ...
  code = expand_vm_options_as_needed(initial_cmd_args, &mod_cmd_args,
                                     &cur_cmd_args);
  if (code != JNI_OK) {
    return code;
  }
  ...
  code =
      expand_vm_options_as_needed(initial_vm_options_args.get(),
  ...
  // Parse JavaVMInitArgs structure passed in, as well as JAVA_TOOL_OPTIONS and
  // _JAVA_OPTIONS
  jint result =
      parse_vm_init_args(cur_vm_options_args, cur_java_tool_options_args,
                         cur_java_options_args, cur_cmd_args);
  ...
  if (needs_module_property_warning) {
    warning("Ignoring system property options whose names match the "
            "'-Djdk.module.*'."
  ...
  if ((UseSharedSpaces && xshare_auto_cmd_line) || log_is_enabled(Info, cds)) {
  ...
  const NMT_TrackingLevel lvl =
      NMTUtil::parse_tracking_level(NativeMemoryTracking);
  if (lvl == NMT_unknown) {
    jio_fprintf(defaultStream::error_stream(),
  ...
  if (PrintNMTStatistics && lvl == NMT_off) {
    warning("PrintNMTStatistics is disabled, because native memory tracking is "
            "not enabled");
  ...
  bool log_class_load_cause =
      log_is_enabled(Info, class, load, cause, native) ||
      log_is_enabled(Info, class, load, cause);
  if (log_class_load_cause && LogClassLoadingCauseFor == nullptr) {
    warning("class load cause logging will not produce output without "
  ...
  if ((HandshakeALot || SafepointALot) &&
      FLAG_IS_DEFAULT(GuaranteedSafepointInterval)) {
...
jint Arguments::apply_ergo() {
  ...
  if (FLAG_IS_DEFAULT(UseSecondarySupersTable)) {
    FLAG_SET_DEFAULT(UseSecondarySupersTable,
                     VM_Version::supports_secondary_supers_table());
  } else if (UseSecondarySupersTable &&
             !VM_Version::supports_secondary_supers_table()) {
  ...
  if (PrintAssembly && FLAG_IS_DEFAULT(DebugNonSafepoints)) {
    warning("PrintAssembly is enabled; turning on DebugNonSafepoints to gain "
            "additional output");
  ...
  if (FLAG_IS_CMDLINE(CompressedClassSpaceSize) &&
      !UseCompressedClassPointers) {
    warning("Setting CompressedClassSpaceSize has no effect when compressed "
            "class pointers are not used");
  }
  ...
  if (BytecodeVerificationLocal && !BytecodeVerificationRemote) {
    log_info(verification)(
        "Turning on remote verification because local verification is on");
  ...
  if (!FLAG_IS_DEFAULT(EnableVectorSupport) && !EnableVectorSupport) {
    if (!FLAG_IS_DEFAULT(EnableVectorReboxing) && EnableVectorReboxing) {
      warning("Disabling EnableVectorReboxing since EnableVectorSupport is "
              "turned off.");
    }
    FLAG_SET_DEFAULT(EnableVectorReboxing, false);

    if (!FLAG_IS_DEFAULT(EnableVectorAggressiveReboxing) &&
        EnableVectorAggressiveReboxing) {
      if (!EnableVectorReboxing) {
        warning("Disabling EnableVectorAggressiveReboxing since "
                "EnableVectorReboxing is turned off.");
      } else {
        warning("Disabling EnableVectorAggressiveReboxing since "
                "EnableVectorSupport is turned off.");
      }
    }
    ...
    if (!FLAG_IS_DEFAULT(UseVectorStubs) && UseVectorStubs) {
      warning(
          "Disabling UseVectorStubs since EnableVectorSupport is turned off.");
    }
  ...
  if (FLAG_IS_CMDLINE(DiagnoseSyncOnValueBasedClasses)) {
    if (DiagnoseSyncOnValueBasedClasses == ObjectSynchronizer::LOG_WARNING &&
        !log_is_enabled(Info, valuebasedclasses)) {
      LogConfiguration::configure_stdout(LogLevel::Info, true,
                                         LOG_TAGS(valuebasedclasses));
    }
  }
  return JNI_OK;
}
...
const char *Arguments::PropertyList_get_value(SystemProperty *pl,
                                              const char *key) {
...
// Return the value of the requested property provided that it is a readable
// property.
const char *Arguments::PropertyList_get_readable_value(SystemProperty *pl,
                                                       const char *key) {
  assert(key != nullptr, "just checking");
  SystemProperty *prop;
  // Return the property value if the keys match and the property is not
  // internal or it's the special internal property
  ...
  for (prop = pl; prop != nullptr; prop = prop->next()) {
    if (strcmp(key, prop->key()) == 0) {
      if (!prop->internal()) {
        ...
        // Property is internal and not jdk.boot.class.path.append so return
        // null.
        return nullptr;
      }
    }
...
void Arguments::PropertyList_add(SystemProperty **plist,
                                 SystemProperty *new_p) {
...
void Arguments::PropertyList_add(SystemProperty **plist, const char *k,
                                 const char *v, bool writeable, bool internal) {
  if (plist == nullptr)
    return;
...
void Arguments::PropertyList_unique_add(SystemProperty **plist, const char *k,
                                        const char *v,
                                        PropertyAppendable append,
                                        PropertyWriteable writeable,
                                        PropertyInternal internal) {
  if (plist == nullptr)
    return;
...
bool Arguments::copy_expand_pid(const char *src, size_t srclen, char *buf,
                                size_t buflen) {
  const char *p = src;
  char *b = buf;
